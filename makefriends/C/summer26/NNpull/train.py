#!/usr/bin/env python3
"""
Training script for NNpull: VBF Z vs QCD Z+jj classification.
DeepSets over constituents with full pull-vector features; jet scalars exclude NC.
Trained on summer26 Herwig Z samples.

Outputs saved to NNpull/ (directory of this script):
...
def scale_jcs(x_jcs, scaler):
    """Apply scaler to (N, NC_MAX, 4) constituent array."""
    shape = x_jcs.shape
...
def main():
    ...
    print(f'  QCD Z+jj Herwig: {len(x_jet_bkg)} events')

    print('Loading signal ...')
    x_jet_sig, x_jcs_sig, _ = load_features(HERE / SIG_FILE)
    print(f'  VBF Z Herwig: {len(x_jet_sig)} events')

    x_jet_all = np.concatenate([x_jet_bkg, x_jet_sig], axis=0)   # (N, 5)
    x_jcs_all = np.concatenate([x_jcs_bkg, x_jcs_sig], axis=0)   # (N, 80, 5)
    ...
    fig.savefig(FIG_DIR / f'NNpull_{PROCESS}_loss_curve.pdf', bbox_inches='tight')
    plt.close(fig)
    print(f'Saved {FIG_DIR / f"NNpull_{PROCESS}_loss_curve.pdf"}')
