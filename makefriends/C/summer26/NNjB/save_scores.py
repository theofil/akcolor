#!/usr/bin/env python3
"""
Run NNjB inference (leading jet + boson scalars) on this channel's summer26
h5 files (QCD{P}jj / VBF{P}, Herwig + MG5+Pythia) and write the per-event
sigmoid score back into each file.
...
Written dataset:
  NNjB — event-level score (leading jet + constituents + boson scalars)
"""
...
def main():
    ...
    for path in H5_FILES:
        ...
        x_jet_raw, x_jcs_raw, _ = load_features(path)
        scores = run_nn(model, scalers, x_jet_raw, x_jcs_raw, device).astype(np.float32)
