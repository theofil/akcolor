...
BKG_FILE = DATA_DIR / 'QCDZtoInv.root'
SIG_FILE = DATA_DIR / 'VBFZtoInv.root'
...
# no split — H-trained models never saw Z data, full dataset used for inference
X_inf     = X
spva0_inf = spva0
y_inf     = y
print(f'Full Z dataset: {len(y_inf)} events  '
      f'({(y_inf==1).sum()} sig, {(y_inf==0).sum()} bkg)')


# ── per-model inference (H-trained models applied to Z) ────────────────────────
...
ax.set_title('VBF Z vs QCD Z', fontsize=22)
