#!/usr/bin/env python3
"""
Train TransNoJet: constituent features only, no jet-level scalars.

Outputs (written next to this script):
  best_model.pt     — state dict at best val loss
  scaler.pkl        — {'jcs': StandardScaler}
  split_indices.npz — train_idx, val_idx, test_idx  (same seed as NNpull/NNtrans)
  loss_curve.pdf
"""
...
def main():
    ...
    mask_train = (x_jcs_all[idx_train][..., 3] > 0)
    ...
    model = TransNoJet(dropout=DROPOUT, clf_dropout=CLF_DROP).to(device)
    ...
    apply_style(ax, xlabel='Epoch', ylabel='Loss', title='Transformer (no jet)', legend_loc='upper right')
    plt.tight_layout()
