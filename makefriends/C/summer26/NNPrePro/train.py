#!/usr/bin/env python3
"""
Training script for NNPrePro: VBF Z vs QCD Z+jj classification.
DeepSets over constituents using only low-level features (no NC, |t|, θ_s, w).
Trained on summer26 Herwig Z samples.

Outputs saved to NNPrePro/ (directory of this script):
...
def main():
    ...
    fig.savefig(FIG_DIR / f'NNPrePro_{PROCESS}_loss_curve.pdf', bbox_inches='tight')
    plt.close(fig)
    print(f'Saved {FIG_DIR / f"NNPrePro_{PROCESS}_loss_curve.pdf"}')
