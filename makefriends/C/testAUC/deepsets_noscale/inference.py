#!/usr/bin/env python3
"""
Inference for DeepSetsNoScale: ROC curve on held-out test set.

Outputs (next to this script):
  scores.npz               — y_test, nn_scores
  roc_deepsets_noscale.pdf — ROC vs individual jet variables
"""
...
from dataset import load_features, BKG_FILE, SIG_FILE, N_CONSTIT_FEAT
from model import DeepSetsNoScale
...
def main():
    print('Loading data ...')
    x_jcs_bkg, _ = load_features(HERE / BKG_FILE)
    x_jcs_sig, _ = load_features(HERE / SIG_FILE)
    x_jet_bkg = load_jet_features(HERE / BKG_FILE)
    x_jet_sig = load_jet_features(HERE / SIG_FILE)

    x_jcs_all = np.concatenate([x_jcs_bkg, x_jcs_sig], axis=0)
    x_jet_all = np.concatenate([x_jet_bkg, x_jet_sig], axis=0)
    y_all = np.concatenate([np.zeros(len(x_jcs_bkg), dtype=np.float32),
    ...
    mask_test  = (x_jcs_test_raw[..., 2] > 0)   # jcsPt/jetPt fraction at index 2

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model  = DeepSetsNoScale().to(device)
    ...
    print(f'DeepSets (no scale) test AUC = {auc_nn:.4f}')
    ...
    plt.tight_layout()
    out = HERE / 'roc_deepsets_noscale.pdf'
