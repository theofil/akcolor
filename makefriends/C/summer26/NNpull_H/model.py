...
class JetNN(nn.Module):
    """
    DeepSets-style network for VBF vs QCD classification (NNpull variant).
    Full pull-vector constituent features; jet scalars exclude NC.
    ...
    Inputs:
      x_jet : (B, N_JET_FEAT=5)               — jet scalars: |η|, m, |t|, pT, θ_s (normalised)
      x_jcs : (B, NC_MAX, N_CONSTIT_FEAT=4)   — constituent features (normalised)
