#!/usr/bin/env python3
"""
Train DeepSetsNoScale: 3 constituent features (jcsDEta, jcsDPhi, jcsPt/jetPt),
no jcsM, no jcsW, no jet-level scalars.

Outputs (written next to this script):
  best_model.pt     — state dict at best val loss
  scaler.pkl        — {'jcs': StandardScaler}  (fitted on 3 features)
...
def main():
    ...
    print(f'  QCD: {len(x_jcs_bkg)} events')

    print('Loading signal ...')
    x_jcs_sig, _ = load_features(HERE / SIG_FILE)
    print(f'  VBF: {len(x_jcs_sig)} events')
    ...
    # mask: jcsPt/jetPt > 0 (fraction at index 2)
    mask_train = (x_jcs_all[idx_train][..., 2] > 0)
    ...
    model = DeepSetsNoScale(dropout=DROPOUT).to(device)
    ...
    apply_style(ax, xlabel='Epoch', ylabel='Loss', title='DeepSets (no scale)', legend_loc='upper right')
    plt.tight_layout()
