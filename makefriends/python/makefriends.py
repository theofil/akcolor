...
if __name__ == "__main__":
    ...
    jetShapeFold = ROOT.TH2F("jetShapeFold", "jetShapeFold", 100, -0.5, 0.5, 100, -0.5, 0.5)
    jetShapeFold.Sumw2()
    jetShapeBold = ROOT.TH2F("jetShapeBold", "jetShapeBold", 100, -0.5, 0.5, 100, -0.5, 0.5)
    jetShapeBold.Sumw2()

    _rhoEdges = np.linspace(0, 0.5, 31)
    _phiEdges = np.linspace(0, 2*np.pi, 65)
    _nArc     = 12
    jetShapeF = ROOT.TH2Poly("jetShapeF", "jetShapeF", -0.5, 0.5, -0.5, 0.5)
    jetShapeF.Sumw2()
    jetShapeB = ROOT.TH2Poly("jetShapeB", "jetShapeB", -0.5, 0.5, -0.5, 0.5)
    jetShapeB.Sumw2()
    for _iPhi in range(64):
        ...
        for _iRho in range(30):
            ...
            jetShapeF.AddBin(len(_xv), _xv_arr, _yv_arr)
            jetShapeB.AddBin(len(_xv), _xv_arr, _yv_arr)
   ...
   #kWeight
    ...
    for ii, ttree in enumerate(ttrees):
        ...
        for iev in range(start_entry, nentries):
            ...
            # fill jetShapeF/B for mjj>400 events with opposite-eta leading jets
            if t_mjj[0] > 400 and len(jets) >= 2 and jets[0].eta()*jets[1].eta() < 0:
                for jet in jets[0:2]:
                    for jc in jet.constituents():
                        dY   = jc.rapidity() - jet.rapidity()
                        dPhi = deltaPhi(jc.phi(), jet.phi())
                        w    = (jc.pt()/jet.pt()) * np.sqrt(dY**2 + dPhi**2)
                        if jet.eta() > 0:
                            jetShapeF.Fill(dY, dPhi, w)
                            jetShapeFold.Fill(dY, dPhi, w)
                        else:
                            jetShapeB.Fill(dY, dPhi, w)
                            jetShapeBold.Fill(dY, dPhi, w)
