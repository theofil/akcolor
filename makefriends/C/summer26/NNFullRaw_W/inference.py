#!/usr/bin/env python3
"""
Inference script for NNFullRaw (summer26, Herwig Z): ROC curves comparing performance
...
Output:
  ../../figs/summer26/roc_nnfullraw.pdf
"""
...
def main():
    ...
    print(f'NNFullRaw Herwig    AUC = {auc_hw:.4f}')
    print(f'NNFullRaw MG5+Py    AUC = {auc_mg5:.4f}')
    ...
    ax.plot(fpr_hw,  tpr_hw,  linewidth=3, color='black',
            label=f'NNFullRaw Herwig W  AUC={auc_hw:.3f}')
    ax.plot(fpr_mg5, tpr_mg5, linewidth=3, color='#1f77b4', linestyle='--',
            label=f'NNFullRaw MG5+Py Z  AUC={auc_mg5:.3f}')
    ...
    out = FIG_DIR / f'roc_nnfullraw_{PROCESS}.pdf'
