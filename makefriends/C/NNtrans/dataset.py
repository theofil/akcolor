"""
DijetDataset: loads QCDHtoInv.h5 + VBFHtoInv.h5, builds per-event feature tensors.

Jet features (6, leading jet = index 0, randomised):
  [0]  |jetEta| — absolute pseudorapidity
  [1]  jetM     — jet mass (GeV)
  [2]  jetNC    — constituent count
  [3]  jetPVM   — pull vector magnitude |t⃗|
  [4]  jetPt    — transverse momentum (GeV)
  [5]  jetSPVA  — signed pull vector angle

Constituent features per jet (NC_MAX=80, 5 per constituent):
  [0]  jcsDEta — sign-flipped Δη relative to jet axis
  [1]  jcsDPhi — ΔΦ relative to jet axis
  [2]  jcsM    — constituent mass (GeV)
  [3]  jcsPt   — constituent pT (GeV)  [used as validity mask: > 0]
  [4]  jcsW    — pull-vector weight
...
N_JET_FEAT     = len(JET_FEATURES)      # 6
N_CONSTIT_FEAT = len(CONSTIT_FEATURES)  # 5
NC_MAX         = 80
...
def load_features(h5path):
    """Return x_jet (N, 6), x_jcs (N, NC_MAX, 5), and kWeight (N,)."""
