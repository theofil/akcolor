#!/usr/bin/env python3
"""
Training script for NNjjZ: VBF Z vs QCD Z+jj classification.
DeepSets over constituents of both jets using only low-level features,
plus the generator-boson 4-vector (pT, η, φ, M) as event-level scalars.
Trained on summer26 Herwig Z samples.

Outputs saved to NNjjZ/ (directory of this script):
  best_model.pt      — state dict at best validation loss
  scaler.pkl         — dict {'jet0', 'jet1', 'boson', 'jcs0', 'jcs1'}: StandardScalers
...
def main():
    ...
    x_j0_bkg, x_j1_bkg, x_bos_bkg, x_c0_bkg, x_c1_bkg, _ = load_features(HERE / BKG_FILE)
    print(f'  QCD Z+jj Herwig: {len(x_j0_bkg)} events')

    print('Loading signal ...')
    x_j0_sig, x_j1_sig, x_bos_sig, x_c0_sig, x_c1_sig, _ = load_features(HERE / SIG_FILE)
    print(f'  VBF Z Herwig: {len(x_j0_sig)} events')
    ...
    x_c0_all = np.concatenate([x_c0_bkg, x_c0_sig], axis=0)   # (N, 80, 4)
    x_c1_all = np.concatenate([x_c1_bkg, x_c1_sig], axis=0)   # (N, 80, 4)
    ...
    with open(HERE / f'scaler_{PROCESS}.pkl', 'wb') as fh:
        pickle.dump({'jet0': scaler_j0, 'jet1': scaler_j1, 'boson': scaler_bos,
                     'jcs0': scaler_c0, 'jcs1': scaler_c1}, fh)

    y_train = y_all[idx_train]
    y_val   = y_all[idx_val]

    train_ds = DijetDataset(x_j0_train, x_j1_train, x_bos_train, x_c0_train, x_c1_train,
                            mask0_train, mask1_train, y_train)
    val_ds   = DijetDataset(x_j0_val,   x_j1_val,   x_bos_val,   x_c0_val,   x_c1_val,
                            mask0_val,   mask1_val,   y_val)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        model.train()
        running = 0.0
        for x_j0_b, x_j1_b, x_bos_b, x_c0_b, x_c1_b, m0_b, m1_b, yb, _ in train_loader:
            x_j0_b = x_j0_b.to(device); x_j1_b = x_j1_b.to(device)
            x_bos_b = x_bos_b.to(device)
            x_c0_b = x_c0_b.to(device); x_c1_b = x_c1_b.to(device)
            m0_b   = m0_b.to(device);   m1_b   = m1_b.to(device)
            yb     = yb.to(device)
            optimizer.zero_grad()
            loss = criterion(model(x_j0_b, x_j1_b, x_bos_b, x_c0_b, x_c1_b, m0_b, m1_b), yb)
            loss.backward()
            optimizer.step()
        ...
        running = 0.0
        with torch.no_grad():
            for x_j0_b, x_j1_b, x_bos_b, x_c0_b, x_c1_b, m0_b, m1_b, yb, _ in val_loader:
                x_j0_b = x_j0_b.to(device); x_j1_b = x_j1_b.to(device)
                x_bos_b = x_bos_b.to(device)
                x_c0_b = x_c0_b.to(device); x_c1_b = x_c1_b.to(device)
                m0_b   = m0_b.to(device);   m1_b   = m1_b.to(device)
                yb     = yb.to(device)
                running += criterion(model(x_j0_b, x_j1_b, x_bos_b, x_c0_b, x_c1_b, m0_b, m1_b), yb).item() * len(yb)
    ...
    fig.savefig(FIG_DIR / f'NNjjZ_{PROCESS}_loss_curve.pdf', bbox_inches='tight')
    plt.close(fig)
    print(f'Saved {FIG_DIR / f"NNjjZ_{PROCESS}_loss_curve.pdf"}')
