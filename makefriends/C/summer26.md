...
All three NNs use the LCG_106_cuda environment. Training is submitted to a GPU node
via HTCondor. To submit:

```bash
condor_submit summer26/NNkin/train_gpu.sub
condor_submit summer26/NNPrePro/train_gpu.sub
condor_submit summer26/NNpull/train_gpu.sub
condor_submit summer26/NNRaw/train_gpu.sub
condor_submit summer26/NNFullRaw/train_gpu.sub
condor_submit summer26/NNFullRaw_H/train_gpu.sub
```
...
cd summer26/NNPrePro   && python train.py
cd summer26/NNpull     && python train.py
cd summer26/NNRaw      && python train.py
cd summer26/NNFullRaw  && python train.py
cd summer26/NNFullRaw_H && python train.py
```
...
### NNPrePro (`summer26/NNPrePro/`)

DeepSets over constituents using only low-level features — no pull-vector jet
scalars (NC, |t⃗|, θ_s removed) and no constituent weight (w removed).
...
### NNpull (`summer26/NNpull/`)

DeepSets over constituents with full pull-vector information; NC removed from
jet scalars.

**Jet scalars** (5, leading jet): |η|, m, |t|, pT, θ_s  
**Constituent features** (80 max × 4): Δη, ΔΦ, pT/pT_jet, w

Architecture:
- **phi MLP**: 4 → 64 → 64 (ReLU, shared across constituents)
- masked-sum pooling → (B, 64)
- **rho MLP**: 69 → 128 → 64 → 1 (BatchNorm + ReLU + Dropout(0.3))

Trained on **QCDZjj_herwig + VBFZ_herwig**.

### NNRaw (`summer26/NNRaw/`)

DeepSets over constituents of the **leading jet** using only raw low-level features —
no pull-vector scalars (NC, |t⃗|, θ_s removed), no constituent weight (w removed),
and signed η (not |η|) kept as a jet scalar.

**Jet scalars** (3, leading jet): η (signed), m, pT  
**Constituent features** (80 max × 3): Δη_raw, ΔΦ, pT/pT_jet
...
### NNFullRaw (`summer26/NNFullRaw/`)

Extension of NNRaw that uses the same low-level features for **both** the leading
(index 0) and sub-leading (index 1) jets. The phi MLP is shared across all
...
condor_submit summer26/NNPrePro/infer_gpu.sub
condor_submit summer26/NNpull/infer_gpu.sub
condor_submit summer26/NNRaw/infer_gpu.sub
condor_submit summer26/NNFullRaw/infer_gpu.sub
condor_submit summer26/NNFullRaw_H/infer_gpu.sub
condor_submit summer26/NNkin_H/infer_gpu.sub
condor_submit summer26/NNkin_W/infer_gpu.sub
condor_submit summer26/NNPrePro_H/infer_gpu.sub
condor_submit summer26/NNPrePro_W/infer_gpu.sub
condor_submit summer26/NNpull_H/infer_gpu.sub
condor_submit summer26/NNpull_W/infer_gpu.sub
condor_submit summer26/NNRaw_H/infer_gpu.sub
condor_submit summer26/NNRaw_W/infer_gpu.sub
condor_submit summer26/NNFullRaw_W/infer_gpu.sub
```
...
## NNPrePro purity study — optimal cut on jet-0 score
...
Scanning NNPrePro_jet0 > threshold to maximise S/(S+B), requiring at least
...
## SR_Run3 optimization — per-channel (mjj, |ΔYjj|, NNPrePro_jet0) cuts
...
steps, NNPrePro_jet0 threshold 0–1 in 0.01 steps.
...
same SR — largely uncorrelated with the jet-0 |θs| (the NNPrePro cut acts on
jet 0 only).
...
└── summer26/               # analysis code
    ...
    │   └── train_gpu.sub   # HTCondor submission
    ├── NNpull/
    │   ├── dataset.py      # jet scalars (5: |η|,m,|t|,pT,θ_s) + constituents (4: Δη,ΔΦ,pT/pT_jet,w)
    │   ├── model.py        # JetNN: phi(4→64→64) + masked pool + rho(69→128→64→1)
    │   ├── train.py        # training loop
    │   ├── inference.py    # ROC on Herwig test + MG5 transfer test
    │   ├── style.py        # matplotlib style helpers
    │   ├── train_gpu.sh    # LCG_106_cuda wrapper
    │   └── train_gpu.sub   # HTCondor submission
    ├── NNRaw/
    │   ├── dataset.py      # jet scalars (3: η,m,pT) + constituents (3: Δη_raw,ΔΦ,pT/pT_jet), leading jet only
    │   ├── model.py        # JetNN: phi(3→64→64) + masked pool + rho(67→128→64→1)
    │   ├── train.py        # training loop
    │   ├── inference.py    # ROC on Herwig test + MG5 transfer test
    │   ├── style.py        # matplotlib style helpers
    ...
    │   └── train_gpu.sub   # HTCondor submission
    └── NNFullRaw_H/
        ├── dataset.py      # same as NNFullRaw but uses QCDHjj_herwig + VBFH_herwig
        ├── model.py        # identical architecture to NNFullRaw
        ├── train.py        # PROCESS="H"; outputs best_model_H.pt, scaler_H.pkl, split_indices_H.npz
        ├── inference.py    # ROC on Herwig H test + MG5 H transfer test
        ├── style.py        # matplotlib style helpers
        ├── train_gpu.sh    # LCG_106_cuda wrapper
        └── train_gpu.sub   # HTCondor submission
```
