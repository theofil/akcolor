...
class JetNN(nn.Module):
    """
    DeepSets-style network for VBF vs QCD classification (NNFullRaw variant).
    Uses the same low-level features as NNRaw but for both the leading and
    sub-leading jets. The phi MLP is shared across both jets' constituents.

    phi: shared MLP applied to each constituent → masked sum pooling per jet
    rho: MLP on [pool0, pool1, jet0_scalars, jet1_scalars] → logit
    ...
    def __init__(self, dropout=0.3):
        ...
        # 64 (pool jet0) + 64 (pool jet1) + 3 (jet0 scalars) + 3 (jet1 scalars) = 134
        self.rho = nn.Sequential(
            nn.Linear(64 + 64 + N_JET_FEAT + N_JET_FEAT, 128), nn.BatchNorm1d(128), nn.ReLU(), nn.Dropout(dropout),
    ...
    def forward(self, x_jet0, x_jet1, x_jcs0, x_jcs1, mask0, mask1):
        pool0 = self._pool(x_jcs0, mask0)   # (B, 64)
        pool1 = self._pool(x_jcs1, mask1)   # (B, 64)
        cat   = torch.cat([pool0, pool1, x_jet0, x_jet1], dim=1)   # (B, 134)
        return self.rho(cat)
